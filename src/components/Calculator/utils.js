...
export const formatNumberToFixed = (number) => {
  const splitNum = String(number).split('.');
...
export const roundMinMax = (isFiat, value, isMax) => {
  if (isFiat) {
    return isMax ? Math.floor(value * 100) / 100 : Math.ceil(value * 100) / 100;
  }

  return isMax ? Math.floor(value * 100000) / 100000 : Math.ceil(value * 100000) / 100000;
};
